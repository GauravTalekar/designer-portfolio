...
const SearchFilter = ({ q = "", type = "", sort = "desc" }) => {
  return (
    <form
      method="GET"
      className="flex w-full flex-wrap gap-4 items-end font-nunito"
      aria-label="Project filter form"
    >
      <fieldset className="flex flex-col w-full md:w-2/5">
        <legend className="sr-only">Search by Name</legend>
        <label htmlFor="q" className="text-sm font-montserrat font-medium">
          Search by Name
        </label>
        <input
          type="text"
      ...
      <fieldset className="flex flex-col w-full md:w-1/5">
        <legend className="sr-only">Filter by Type</legend>
        <label htmlFor="type" className="text-sm font-montserrat font-medium">
          Filter by Type
        </label>
        <select
          name="type"
        ...
        >
          ...
          {projectTypeFilter.map((type, idx) => (
      ...
      <fieldset className="flex flex-col w-full md:w-1/5">
        <legend className="sr-only">Sort by Date</legend>
        <label htmlFor="sort" className="text-sm font-montserrat font-medium">
          Sort by Date
        </label>
        <select
          name="sort"
